 ...
 *
 * Fixes homophones that speech-to-text gets wrong:
 *   "to"   → "two"
 *   "too"  → "two"
 *   "tree" → "three"
 *   "for"  → "four"
...
const Voice = (() => {
  ...
  // These are words speech-to-text commonly returns instead of number words.
  // Applied as whole-word replacements BEFORE parsing.
  const HOMOPHONES = {
    'to'   : 'two',
    'too'  : 'two',
    'tree' : 'three',
    'free' : 'three',  // Indian accent sometimes
    'for'  : 'four',
    'fore' : 'four',
    'ate'  : 'eight',
    'won'  : 'one',
    'on'   : 'one',    // "on" at end of chunk is likely "one"
    'nein' : 'nine',
    'sex'  : 'six',
    'sick' : 'six',    // Indian accent
    'to'   : 'two',
    'tent' : 'ten',
  };
  ...
  /**
   * Fix homophones in a transcript string.
   * Only replaces whole words so product names are not affected.
   * e.g. "taaza two aur tree biscuit three" → "taaza two aur three biscuit three"
   * Wait — "tree" as product name would break. So we ONLY fix the last
   * token of each chunk (which is the qty position), not inside names.
   * We do a conservative pass: replace only when surrounded by spaces or
   * at start/end of string, AND the replacement is a number word.
   */
  function _fixHomophones(transcript) {
    // Replace whole-word homophones globally first pass
  ...
  //  Output: [{rawName:"gold",qty:1},{rawName:"taaza",qty:2},{rawName:"parle g",qty:3}]
  // ═══════════════════════════════════════════════════════════════════

  function parseTranscript(transcript) {
    const fixed = _fixHomophones(transcript);
    console.log('[Voice] After homophone fix:', fixed);
    ...
    for (const chunk of chunks) {
      ...
      if (qty !== null && tokens.length > 1) {
        ...
        // No number at end — whole chunk is name, qty=1
  ...
  function _initRecognition() {
    ...
    if (!SR) { _disableVoiceBtn(); return false; }
    ...
    _recognition.onresult = e => {
      ...
      if (interim && !final) _setTranscript(interim, 'interim');
      if (final) { _transcript = final.trim(); _setTranscript(_transcript, 'final'); }
    };

    _recognition.onerror = e => {
      const msgs = { 'no-speech':'No speech — try again', 'audio-capture':'Mic unavailable',
  ...
  function _onHoldStart(e) {
    ...
    _holdTimer  = setTimeout(() => {
      ...
      try { _recognition.start(); } catch (err) { console.warn('[Voice] Start:', err); }
    }, MIN_HOLD_MS);
  }
  ...
  function _onHoldEnd(e) {
    ...
    if (_isListening) {
      try { _recognition.stop(); } catch (err) { console.warn('[Voice] Stop:', err); }
    } else {
      _updateVoiceBtnState(false);
    }
  }
  ...
  function _updateVoiceBtnState(listening) {
    const btn = $('voice-btn'), label = $('voice-label');
    if (!btn) return;
    btn.classList.toggle('listening', listening);
    btn.setAttribute('aria-label', listening ? 'Release to process' : 'Hold to speak');
    if (label) label.textContent = listening ? 'Listening…' : 'Hold to Speak';
  }
